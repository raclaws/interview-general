...
- **hx-boost makes ALL child links HTMX-managed** — any `<a href>` inside a boosted container (including `#main`) gets intercepted. Links that navigate to form pages, download files (export), or open external URLs MUST have `hx-boost="false"`. This includes: Edit links, + New/Add links, Cancel links, export PDF/CSV, and scorecard links. Only SPA page-to-page navigation (list→detail, breadcrumbs) should remain boosted.
